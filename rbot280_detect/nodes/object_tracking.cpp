...
void ObjectTracking::PcImageFusionCb(const sensor_msgs::ImageConstPtr& in_img_msg,
...
void ObjectTracking::ClassificationCb(const rbot_msgs::ClassificationArray &in_classification_msg)
{
  //  for( int i=0; i<in_classification_msg.classifications.size(); i++)
  //{
  //  ROS_INFO(in_classification_msg.classifications[i].label.c_str());
  //}
  ROS_INFO("size %ld ", in_classification_msg.classifications.size());

}

pcl::PointXYZ ObjectTracking::TfPoint(const pcl::PointXYZ &in_point,
...
tf::StampedTransform ObjectTracking::GetTransform(const std::string& in_target_frame,
...
void ObjectTracking::IntrinsicsCb(const sensor_msgs::CameraInfoConstPtr& info_msg)
{
...
void ObjectTracking::InitRosIO(ros::NodeHandle &in_private_nh)
{
  std::string img_src, img_info_src, cloud_src, classification_src;
  ...
  in_private_nh.getParam("classification_src", classification_src);
  ROS_INFO("[%s] classification_src %s", __APP_NAME__, classification_src.c_str());
  
  // Subscriptions
  image_intrinsics_sub_ = in_private_nh.subscribe(img_info_src, 1,
  						  &ObjectTracking::IntrinsicsCb, this);
  ...
  ROS_INFO("[%s] classification_sub_ subscribing to %s", __APP_NAME__, classification_src.c_str());
  
  classification_sub_ = in_private_nh.subscribe(classification_src.c_str(), 1,
						&ObjectTracking::ClassificationCb, this);
  ...
  pc_cam_sync_->registerCallback(boost::bind(&ObjectTracking::PcImageFusionCb, this, _1, _2));

  // Publishers
  image_pub_ = it_.advertise(img_src + "/object_tracking", 1);
}
